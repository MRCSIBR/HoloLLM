r"""
SCRIPT: train_350m_streaming.py
PROYECTO: HoloLLM (MRCSIBR/HoloLLM) - Escalamiento 350M Canónico
DESCRIPCIÓN: Entrenamiento masivo a 7800 tok/s con inicialización std=0.02 y corpus híbrido.
"""
...
CONFIG = {
    ...
    "vocab_size": 151936,
    "dim": 768,               # 350M Escala
    "depth": 16,              # 16 Capas Bulk
    "num_heads": 12,          # 12 Cabezas (d_h = 64)
    "seq_len": 512,
    "batch_size": 8,          # 4096 tokens por paso
    "max_steps": 3000,
    "lr": 3e-4,               # Learning rate saludable para entrenamiento canónico
    "min_lr": 1e-5,
    "warmup_steps": 150,
    "temperature": 1.0,
    "alpha_distill": 0.5,
...
def load_hybrid_dataset() -> List[Tuple[str, str]]:
    ...
    # Inyectamos con alta frecuencia las 10 joyas canónicas
    for _ in range(50):
...
def main() -> None:
    ...
    print("=" * 80)
    print(f"HOLO-LLM 350M: CORRIDA DEFINITIVA (INICIALIZACIÓN CANÓNICA std=0.02)")
    print(f"Dispositivo: {device} | Memoria: O(1) Constante (0 KB KV-Cache)")
    print("=" * 80)

    # 1. Maestro Jan-Code-4B
    ...
    print(f"[Teacher] Maestro activo en VRAM (25 GB).")

    # 2. Alumno HoloLLM-350M con inicialización std=0.02
    ...
    print(f"[Student] Inicializado con std=0.02 (Varianza natural).")
    print(f"[Student] std real de Embedding: {student.token_emb.weight.std().item():.4f}")
    ...
    print("=" * 80)
    print("INICIO DE ENTRENAMIENTO: ESPERAMOS PÉRDIDA INICIAL ~11.9 (NO 281)")
    print("=" * 80)
    ...
    for step in range(1, CONFIG["max_steps"] + 1):
        ...
        if step % 25 == 0 or step == 1:
            ...
            print(f"Paso 350M [{step:04d}/{CONFIG['max_steps']:04d}] | "
        ...
        if step % CONFIG["save_interval"] == 0 or step == CONFIG["max_steps"]:
            ...
            print(f"\n>>> Checkpoint 350M guardado: {CONFIG['checkpoint_path']} (SHA: {sha[:12]})\n", flush=True)

    for h in hook_handles:
        h.remove()
    print("[HoloLLM-350M] Entrenamiento completado con éxito.", flush=True)
